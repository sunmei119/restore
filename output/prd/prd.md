...
| 模块 | 包含功能 |
|------|---------|
| 应用管理 | 应用申请管理、应用上架管理、上架范围管理、应用分类管理、应用审核管理、标签管理 |
| 平台管理 | 应用统计 |
| 订单管理 | 订单套餐授权管理、订单接收记录管理、试用套餐记录管理、应用套餐配置 |
...
2. 管理员在审核列表的"待审批"Tab 中看到该记录。管理员查看应用详情后，选择"通过"或"驳回"。通过后 app.status 变为 online，应用出现在上架管理列表中；驳回时需填写驳回原因，review.status 变为 rejected，app.status 保持 offline。
...
flowchart TD
    ...
    D --> E["应用基本信息锁定不可编辑"]
    E --> F["管理员在审核列表待审批Tab 查看"]
    F --> G{"审核结论"}
    G -->|"通过"| H["app.status = online<br/>进入上架管理列表"]
...
1. 管理员对已上线（online）且未上架（unlisted）的应用发起上架，进入上架配置页，配置上架范围（全平台或按范围选择），阅读并勾选上架承诺书后确认。确认后 app.listing_status 变为 listed。已上架的应用，点击对应的上架范围，打开应用安装列表，点击机构对应的安装按钮为单个安装，点击全部安装则安装到当前安装列表的所有租户下，卸载和全部卸载通用的逻辑；
2. 安装的流程：非低代码应用安装，只安装到应用中心即让应用在机构可见，因为不像低代码有安装包；低代码应用安装分跨租户和不跨租户安装，跨租户安装同非低代码流程；不跨租户安装，会先安装到应用中心，如果应用中心安装失败，则状态变为安装失败，点击安装按钮可重新安装；应用中心安装成功后，再安装到低代码平台，若安装成功则非跨租户的低代码应用安装成功；若低代码平台安装失败则状态为安装失败，可点击重试按钮，继续安装到低代码平台。
...
flowchart TD
    ...
    H -->|低代码 跨租户| I
    H -->|低代码 非跨租户| J[先安装到应用中心]
    J --> J1{应用中心安装?}
    J1 -->|失败| J2[状态=安装失败<br/>可点击重试]
    J1 -->|成功| K[再安装到低代码平台]
    K --> K1{低代码平台安装?}
    K1 -->|成功| K2[非跨租户低代码应用安装成功]
...
点击免费试用，生成一条试用订单记录；点击立即购买打开购买界面，填入发票信息，点击提交订单跳转到付费界面，展示跟惠缴费对接的二维码，手机扫码唤起惠缴费半屏支付，支付成功后PC端支付状态同步提示支付成功，支付失败则提示失败，同时出现返回订单的按钮点击返回我的订单界面
...
用户进入页面后首先看到当前筛选条件下的应用列表，可以直接判断每条应用的上下线状态和基本属性。列表默认展示：应用 Logo（32×32 圆角图标）、应用名称（超 20 字符末尾截断，悬停显示全文）、应用分类（一级分类为主标，二级分类为副标灰字）、应用类型（平台应用/非平台应用）、应用属性（完整属性文本，如"原生应用""原生应用/低代码应用""生态应用"）、适用机构（多值枚举展示，如"学校""上级单位"）、适用学段（多值徽标展示，仅适用机构包含学校时显示）、状态标签（已上线绿色徽标，未上线灰色徽标）。
...
点击"确认上线"前系统校验应用的 Logo、名称、分类等必填项是否完整。校验不通过时顶部红色提示"应用信息不完整，请先完善"，阻止提交。校验通过后系统生成一条审核记录（review.type = 上线，review.status = pending，review.applicant = 当前登录人），toast 提示"上线申请已提交"，返回列表页。

网络异常导致提交失败时，顶部红色提示"提交失败，请检查网络后重试"。

（4）管理应用

仅 app.status = online 的应用行显示"管理"按钮。点击后进入 P02b 应用详情页（管理模式），可查看凭证区和完整信息，并通过"修改"链接跳转 P02 编辑模式。
...
| 应用类型 | * | 单选 | 平台应用 / 非平台应用；仅桂教通超级管理员展示且必填，租户管理员新增应用默认非平台应用（不展示此字段） |
| 是否数据同步 | * | 单选 | 否 / 是，默认否 |
...
点击"新建"触发全量校验，未通过时页面滚动定位到首个错误字段。校验通过后写入 app 记录，系统自动生成 app.system_suite_id（全局唯一，不可修改）与 app.system_suite_key，同时生成 review 记录（review.status = pending），toast 提示"上线申请已提交"，返回 P01 列表。
...
页面由面包屑、头部凭证区、基本信息区和底部操作区组成。根据进入入口分为四种模式：管理模式（P01"管理"进入，仅 online 应用）、上线申请模式（P01"上线"进入，仅 offline 应用）、只读查看模式（P01 其他用户"查看"进入）、审核详情模式（P10"详情"进入）。
...
**上线申请模式**：显示「提交」（主）+「返回」（次）。点击"提交"前系统核对应用必填字段完整性。缺项时顶部红色提示"应用信息不完整，请先完善"，阻止提交。校验通过后系统生成 review 记录（review.type = 上线，review.status = pending），toast 提示"上线申请已提交"，返回 P01 列表。
...
列表展示所有上线申请的操作记录：应用名称、申请类型（上线/下线）、状态徽标（待审核/审核通过/审核驳回）、申请人、申请时间。按申请时间倒序排列，每页 20 条。无记录时显示"暂无操作记录"。

（2）查看详情

点击行项弹出详情浮层，展示：应用名称、申请类型、状态、申请人、申请时间、审核人（审核通过后显示）、审核时间、审核备注（驳回时显示驳回原因）。
...
列表按创建时间倒序，每页 20 条，展示：应用 Logo、应用名称、应用分类（一级/二级）、应用类型（平台应用/非平台应用）、开发者名称、应用来源（当前租户名称）、应用属性（完整属性文本，如"原生应用""原生应用/低代码应用""生态应用"）、上架范围（当前应用关联的范围名称列表，超链接形式可点击进入 P04b）、创建时间、上架状态（已上架/已下架/未上架标签）。
...
由 P04"上架"按钮进入，页面分为应用基本信息区（只读）、上架配置区、免责声明与承诺书三部分。底部按钮居中："取消"和"确认上架"。

> 上架配置区按"应用是否为低代码应用"（由应用信息的应用属性判断，非手动切换）与"操作人角色"展示不同字段；当前操作角色通过页面右上角的角色选择器切换。
> - **低代码应用 + 桂教通超级管理员**：展示 局校公用（选"是"展示跨租户层级、所属租户，且范围选择仅显示"按范围"；选"否"范围选择显示"全平台 / 按范围"）/ 范围选择（全平台 / 按范围）。局校公用指将应用上架范围与跨租户范围保持一致，并自动安装到范围内租户（仅安装到应用中心）。
> - **低代码应用 + 租户管理员**：展示 局校公用（选"是"时不展示范围选择；选"否"展示范围选择：全平台/按范围，必填）/ 范围选择（全平台 / 按范围，数据来源本租户上架范围管理）/ 分享范围（多选：桂教通平台/其他机构，非必填；勾选后展示版本号）。不展示数据同步、应用所属厂商、是否自动安装、是否可试用、收费方式及其子项。
> - **非低代码应用 + 桂教通超级管理员**：展示 收费方式（选"收费"时展示是否可试用、试用套餐、授权套餐）/ 范围选择（全平台 / 按范围）。不展示分享范围。
> - **非低代码应用 + 租户管理员**：展示 收费方式（选"收费"时展示是否可试用、试用套餐、授权套餐）/ 范围选择（全平台 / 按范围，必填）/ 分享范围（多选：桂教通平台/其他机构，非必填；勾选后展示版本号）。
> - **范围选择（按角色区分）**：桂教通超级管理员：跨租户=否时全平台/按范围（必填），跨租户=是时仅按范围；租户管理员：全平台/按范围（必填，全平台=上架到当前租户及所有下级租户并自动安装应用中心，对低代码/非低代码均展示）。
> - **分享范围 / 版本号**：租户管理员（低代码/非低代码均展示）。分享范围多选（桂教通平台/其他机构，非必填）；勾选分享范围后展示版本号（下拉搜索框，单选，必填），按输入的应用ID获取对应版本号，未获取到则提示"需要上架应用到低代码应用市场"。
...
| 字段 | 控件 | 必填 | 联动规则 |
|------|------|:----:|---------|
| 局校公用 | radio 是/否 | * | 仅低代码应用展示；桂教通超级管理员选"是"时展示跨租户层级和所属租户，租户管理员选"是"时不展示这两项 |
| 跨租户层级 | radio 厅/市/区县 | * | 仅桂教通超级管理员 + 低代码应用展示；仅跨租户=是时显示（已删除"乡镇集团校中心校"） |
| 所属租户 | 下拉选择（支持搜索）+ 添加按钮 | * | 仅桂教通超级管理员 + 低代码应用展示；仅跨租户=是时显示；点击"添加"弹出机构选择框 |
| 范围选择 | radio 全平台/按范围 | *（租户管理员必填） | 桂教通超级管理员：跨租户=否时全平台/按范围（必填），跨租户=是时仅按范围；租户管理员：全平台/按范围（必填，对低代码/非低代码均展示） |
| 分享范围 | checkbox 桂教通平台/其他机构 | 否 | 租户管理员展示（低代码/非低代码均展示）；多选、非必填 |
| 版本号 | 下拉搜索框（单选） | *（勾选分享范围时） | 租户管理员且已勾选分享范围时显示；按输入应用ID远程获取版本号；未获取到则提示"需要上架应用到低代码应用市场" |
| 是否可试用 | radio 是/否 | * | 仅非低代码应用展示；仅收费方式=收费时显示；选"是"时展示试用套餐下拉 |
| 试用套餐 | 下拉 | * | 是否可试用=是且收费方式=收费时显示 |
| 收费方式 | radio 收费/免费 | * | 仅非低代码应用展示；选"收费"时展示授权套餐 |
| 授权套餐 | 下拉框（数据源：套餐管理，可多选） | * | 收费方式=收费时显示 |

范围选择的具体交互：
- 桂教通超级管理员：跨租户=否时全平台（文本提示"全平台指的是上架到当前机构所有下属机构和学校"）/ 按范围（可搜索下拉框多选，数据来源本租户上架范围管理）；跨租户=是时仅显示"按范围"。
- 租户管理员：全平台/按范围单选、必填（低代码/非低代码均展示）。全平台=上架到当前租户及所有下级租户并自动安装到应用中心；按范围=可搜索下拉框多选（数据来源本租户上架范围管理）。确认上架后按所选范围在应用上架管理新增未上架数据（应用来源 = 当前租户名称）。
- 所属租户「添加」按钮：点击弹出机构选择框，按跨租户层级弹性展示机构——厅级展示厅级机构、市级展示所有市级机构、区县级展示所有区县教育局；机构数 ≤ 10 以列表展示（点击回填输入框），> 10 以下拉选择展示。
- 分享范围 + 版本号（租户管理员，低代码/非低代码均展示）：分享范围多选（桂教通平台/其他机构，非必填）；勾选后展示版本号下拉搜索框（单选、必填），按输入应用ID远程获取版本号，未获取到则提示"需要上架应用到低代码应用市场"。

（2）确认上架

底部承诺书行：单选框 + 文案"我已仔细阅读并完全同意应用免责声明及上架承诺书，自愿承担一切相关法律责任。"点击蓝色链接弹出五条免责声明弹窗，弹窗禁止点击遮罩关闭，仅提供"我知道了"按钮关闭。未勾选时"确认上架"按钮置灰。
...
展示本租户手动创建和维护的范围。列表列：范围名称、范围 ID、范围类型（自定义/按教育机构/按标签）、范围摘要、创建时间、创建人。另有状态列（启用绿色标签/停用红色标签）和操作列（编辑 + 删除仅停用显示 + 停用/启用按钮合并在一列）。删除、启用、停用均需弹窗二次确认。

（2）查看默认范围列表

展示系统自动生成的范围（如教育局市场自动生成的默认范围），列表列：范围名称、范围 ID、创建时间。仅可查看，无编辑或管理操作。
...
- 点击"提交"：审批结果=通过时 review.status = approved，app.status = online，写入 reviewer 和 reviewed_at，toast「审核通过，应用已上线」；审批结果=驳回时 review.status = rejected，app.status 保持 offline，review_comment = 审批意见，toast「已驳回」。
...
展示所有标签：标签 ID、标签名称、标签类型（自定义标签/机构类型标签）、关联范围数量。无标签时显示"暂无标签"。
...
本模块负责订单套餐授权、订单接收、试用订单和应用套餐配置管理，包含 5 个页面。
...
筛选栏：套餐名称（模糊查询）、授权方式（下拉：全部/自动/手动）。列表展示：订单 ID、授权方式（自动/手动标签）、套餐名称、使用范围、业务类型、有效期/天、分配大小、所属机构、购买用户 ID、到期时间、订单接收时间。

（2）手动授权操作

仅授权方式=手动的订单行显示操作按钮：

- **授权**（authStatus=pending 时显示）：点击弹窗二次确认「确认授权【套餐名称】吗？」，确认后授权成功，toast「授权成功」；点击取消不变更。
- **取消授权**（authStatus=authorized 时显示）：点击弹窗二次确认「确认取消授权套餐：XXXX吗？取消后购买该套餐的用户将不可使用。」，确认后取消授权，toast「已取消授权」；点击取消不变更。

#### P19 订单接收记录管理列表页

购买应用商品后生成的订单信息列表。列表列较多，支持横向滚动。

（1）查询和列表

筛选栏：套餐名称（模糊查询）、订单状态（下拉：全部/待支付/已支付/已取消）。
列表展示：订单 ID、套餐名称、购买用户 ID、购买用户名称、购买所属机构、订单状态标签、购买类型（个人/机构）、订单接收时间、授权状态（授权成功/未授权标签）、授权状态描述。按接收时间倒序。
...
列表展示：试用机构名称、用户姓名、手机号码、试用类型、试用期限（天数）、开通时间、到期时间、延期说明、状态标签（使用中绿色/已到期红色/停用黄色）。列表列较多，支持横向滚动。

（2）停用操作

仅使用中状态的记录行显示"停用"按钮。点击弹窗二次确认"停用该账户后，将无法试用{{试用类型}}，请确认是否继续操作？"，确认后状态改为"停用"，收回试用权限。停用的账号不可再申请延期。
...
筛选栏：套餐名称、套餐编码、套餐类型（下拉）。列表展示：序号、套餐类型、套餐编码、套餐名称、价格、适用应用范围、分配大小、计量单位、有效期(天)、基座套餐关联、是否手动开通业务、状态（启用/停用标签）。列表内容较宽，支持横向滚动。
...
| 字段 | 必填 | 控件 | 联动规则 |
|------|:----:|------|---------|
| 套餐类型 | * | 下拉 | 数据来源数据字典：短信充值/存储容量充值/TOKEN充值/基座套餐/应用使用授权/应用试用授权 |
| 套餐编码 | * | 文本框 | 常显；全局唯一校验 |
| 套餐名称 | * | 文本框 | 常显 |
| 价格(元) | * | 数字输入框 | 常显；必填 |
| 适用范围 | * | 文本框 | 常显；套餐类型=应用使用授权、应用试用授权时填写应用ID |
| 分配大小 | 条件必填 | 文本框 | 短信充值/存储容量充值/TOKEN充值类型时必填 |
| 计量单位 |   | 文本框 | 常显（如 G/条/资源点） |
| 有效期(天) | 条件必填 | 数字输入框 | 应用使用授权/应用试用授权/存储容量充值类型时必填 |
| 基座套餐关联 | 条件必填 | 下拉多选 | 套餐类型=基座套餐时必填；数据源为已存在的套餐配置，可多选 |
| 套餐适用对象 |   | 下拉选择 | 字典：机构/个人 |
| 是否手动开通业务 | * | radio | 自动/手动 |

编辑时回填已有数据。底部按钮居中："取消"和"确定保存"。保存时校验必填项（含价格）、套餐编码全局唯一，并按套餐类型校验分配大小、有效期、基座套餐关联。

（3）删除、启用/停用

行内"删除"（弹窗二次确认后删除）、"启用/停用"（弹窗二次确认后切换 is_active）。
...
顶部展示 Logo、名称、应用介绍（可滚动）、收费类型。

（2）安装/购买

- 免费应用：展示"立即安装"按钮，点击后安装，toast"安装成功"。
- 付费应用：以 iframe 内嵌商城商品详情页，隐藏浏览器地址栏、搜索框、标签栏、购物车按钮。支付保持商城现有机制，完成后返回并提示购买成功。
...
3. **我的应用 & 低代码应用（第一行合并模块，Tab 切换）**：
   ...
   - 低代码应用 Tab 展示当前用户所有已安装的低代码应用及自建应用，提供分类切换（全部、教学类、行政类、科研类），切换不同分类展示对应应用（两行图标卡片），默认"全部"；低代码应用若为跨租户应用，图标卡片下方显示来源租户名称标签。
...
| 角色 | 可见菜单 |
|------|----------|
| 租户管理员 | 应用管理、应用申请管理、应用上架管理、上架范围管理、应用分类管理、应用审核管理、标签管理、应用统计看板、桂教通门户-教育服务中心、我的空间-教育服务中心、我的应用中心、我的订单 |
| 桂教通超级管理员 | 应用管理、应用申请管理、应用上架管理、上架范围管理、应用分类管理、应用审核管理、标签管理、应用统计看板、订单套餐授权、订单接收记录、试用订单记录、应用套餐配置 |

> 说明：订单管理（订单套餐授权/订单接收记录/试用订单记录/应用套餐配置）仅桂教通超级管理员可见；桂教通门户（教育服务中心）、我的空间（教育服务中心/我的应用中心/我的订单）仅租户管理员可见。
...
**应用审核操作页（P11）**：
- platform_operator：可审核所有租户的申请
- tenant_admin（审核员身份）：仅可审核本租户的申请
...
| app.app_type | enum | 否 | non_platform / platform；平台应用仅桂教通超级管理员可选，租户管理员新增默认非平台应用（不展示） |
...
| log.action | enum | 是 | submit_review / approve / reject / list / delist / install / uninstall |
...
| package.scope | string | 是 | 适用应用范围；套餐类型=应用使用授权、应用试用授权时填写应用ID |
...
| package.base_pkg_id | array<string> | 否 | 基座套餐关联（可多选）；套餐类型=基座套餐时必填 |
| package.target_type | enum | 否 | 套餐适用对象：机构 / 个人 |
| package.auth_mode | enum | 是 | 是否手动开通业务：自动 / 手动 |
| package.is_active | boolean | 是 | 是否启用，默认 true |
...
| 状态 | 含义 | 操作人 | 触发动作 | 下一状态 | 限制条件 |
|------|------|--------|----------|----------|----------|
| unlisted | 未上架，范围中不可见 | tenant_admin | 上架配置 | listed | app.status 必须为 online；需配置上架范围并勾选承诺书 |
...
| pending | | platform_operator / tenant_admin（审核员） | 审核通过 | approved | — |
| pending | | platform_operator / tenant_admin（审核员） | 驳回 | rejected | 驳回原因必填 |
| approved | 审核通过，应用已上线 | — | — | — | 终态；app.status 自动变为 online |
