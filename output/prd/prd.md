...
教职工/管理员新增应用、上线申请、桂教通超级管理员审批、结果归档三个阶段。

1. 教职工/管理员在应用申请管理页对已创建但未上线（app.status = offline）的应用提交上线申请。提交前确认应用基本信息完整，提交后系统生成一条审批记录（review.status = pending）。此时应用不可编辑基本信息。
...
1. 管理员对已上线（online）且未上架（unlisted）的应用发起上架，进入上架配置页，配置上架范围（全平台或按范围选择），P05 展示上架承诺书文案及链接供查阅后确认。确认后 app.listing_status 变为 listed。已上架的应用，点击对应的上架范围，打开应用安装列表，点击机构对应的安装按钮为单个安装，点击全部安装则安装到当前安装列表的所有机构下，卸载和全部卸载通用的逻辑；
...
模块级权限规则：默认 tenant_admin 与教职工具备本机构应用的管理权限（新增/编辑应用、提交上线申请、上架配置本人应用）；platform_admin 和 platform_operator 仅可查看，不可修改机构应用数据；tenant_user（非教职工）无此模块访问权限。
...
- 所属机构（下拉选择 + 添加按钮，*）：仅桂教通超级管理员 + 低代码应用 + 是否跨机构=是 时显示；点击"添加"弹出机构选择框（按跨机构层级弹性展示机构，≤10 以列表点击回填，>10 以下拉选择）。
...
由 P04"上架"按钮进入，页面分为应用基本信息区（只读）与上架配置区两部分（上架配置区仅保留"范围选择"与"分享范围"；机构管理员且当前机构为学校时，上架配置区不展示"范围选择"，默认上架范围即本校，仅展示"分享范围"；其他机构分享过来的应用上架时不展示"分享范围"，被分享过来的应用不可再被分享给其他机构）；底部按钮居中："取消"和"确认上架"。
...
范围选择的具体交互：
- 桂教通超级管理员：是否跨机构=否时全平台（文本提示"全平台所有机构"）/ 按范围（可搜索下拉框多选，数据来超管的上架范围管理）；是否跨机构=是时仅显示"按范围"。
...
- 超管（platform_admin / platform_operator）：应用安装数（平台应用安装成功数量，累计至当前）、平台应用数（所有上架到市场的应用数量，累计至当前）、活跃应用数（时间范围内至少打开一次的应用数）、应用使用人数（时间范围内至少使用一次平台任意应用的用户总数）。除活跃应用数、应用使用人数随时间切换变化外，其余卡片不随时间切换变化。
- 机构管理员（tenant_admin）：应用安装数（本机构范围内应用安装成功数量，累计至当前）、本机构应用数（本机构创建/管理的应用数量，累计至当前，含未上架/已上架/已下架）、活跃应用数（本机构范围内时间范围内至少打开一次的应用数）、应用使用人数（本机构范围内时间范围内至少使用一次应用的用户总数）。
...
模块级权限规则：platform_admin 和 platform_operator 全部只读；tenant_admin 本机构内查看；套餐配置页 tenant_admin 可管理本机构套餐。
...
展示当前用户头像、用户名、所属机构名称（由统一门户提供）。提供"我的应用中心""我的订单""教育服务中心"快捷链接卡片。
...
顶部提供订单号搜索框。列表以卡片形式展示当前用户/机构的订单：状态标签、订单号、下单时间、实付金额（红色）、商品图标、商品名称、单价×数量，以及"订单详情"按钮。卡片按下单时间倒序。
...
|------|------|---------------|-------------------|-------------|-------------|
| 应用管理 | 应用申请管理页 | 全部只读 | 全部只读 | 本机构管理 | — |
| | 应用申请编辑页 | — | — | 本机构新增/编辑 | — |
| | 应用上架管理页 | 全部只读 | 全部只读 | 本机构管理 | — |
| | 应用安装列表页 | 全部只读 | 全部只读 | 本机构管理 | — |
...
| 订单管理 | 订单套餐授权管理 | 全部只读 | 全部只读 | 本机构查看 | — |
| | 订单记录管理 | 全部只读 | 全部只读 | 本机构查看 | — |
| | 试用套餐记录管理 | 全部只读 | 全部只读 | 本机构查看 | — |
...
|------|-----------|---------------|-------------------|-------------|-------------|
| 应用申请管理页 | 新增/管理/上线/下线 | — | — | 本机构可操作 | — |
| 应用申请编辑页 | 保存/提交 | — | — | 本机构可操作 | — |
| 应用上架管理页 | 上架/下架/安装 | — | — | 本机构可操作 | — |
| 应用审批操作页 | 通过/驳回 | 可审批 | — | — | — |
...
| auth.status | enum | 是 | pending / active / expired / revoked |
