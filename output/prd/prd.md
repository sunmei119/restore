...
本文档为应用中心重构的产品需求规格说明，面向多租户应用全生命周期管理平台，覆盖租户内应用申请→审批→上架→范围→安装、平台级应用监管与运营统计、教育服务中心集成以及订单管理的完整功能定义。
...
本模块负责租户内应用的申请、上架、范围配置、分类管理和审批全流程，包含 12 个页面。
...
**上线申请模式**：应用基本信息区下方（同一模块内）新增可编辑「上线配置」区（与 P05 原上架配置一致，但仅含除"范围选择""分享范围"以外的配置项），提交/返回按钮统一置于模块最下方，显示「提交」（主）+「返回」（次）。点击"提交"前系统核对应用必填字段完整性及上线配置必填项，且须勾选上架承诺书（未勾选时"提交"置灰）；缺项时顶部红色提示"应用信息不完整，请先完善"，阻止提交。校验通过后系统生成 review 记录（review.type = 上线，review.status = pending），toast 提示"上线申请已提交，待桂教通超级管理员审批"，返回 P01 列表。
...
由 P04"上架"按钮进入，页面分为应用基本信息区（只读）与上架配置区两部分（上架配置区仅保留"范围选择"与"分享范围"；机构管理员且当前机构为学校时，上架配置区不展示"范围选择"，仅展示"分享范围"）；底部按钮居中："取消"和"确认上架"。

> - **范围选择**：桂教通超级管理员：全平台/按范围（必填）；机构管理员：全平台/按范围（必填，对低代码/非低代码均展示，全平台=上架到当前机构及所有下级机构并自动安装应用中心）；机构管理员且当前机构为学校时不展示范围选择（学校无下级机构，全平台/按范围不适用）。
> - **分享范围**：仅机构管理员展示（低代码/非低代码均展示）；多选（桂教通平台/其他机构，学校机构必填、其余非必填）；勾选"其他机构"时联动展示机构模糊搜索多选框（输入机构名称模糊筛选、最多 50 条、可多选）。当前机构为学校时：所选"桂教通平台"或"其他机构"会在桂教通超管与对应机构的应用上架管理界面各新增一条未上架记录。
...
| 字段 | 控件 | 必填 | 联动规则 |
|------|------|:----:|---------|
| 范围选择 | radio 全平台/按范围 | *（机构管理员必填） | 桂教通超级管理员：是否跨机构=否时全平台/按范围（必填），是否跨机构=是时仅按范围；机构管理员：全平台/按范围（必填，对低代码/非低代码均展示）；机构管理员且当前机构为学校时不展示该字段 |
| 分享范围 | checkbox 桂教通平台/其他机构 | 学校机构必填 | 机构管理员展示（低代码/非低代码均展示）；多选；勾选"其他机构"时联动展示"其他机构"机构多选框（模糊搜索、最多 50 条、可多选）；当前机构为学校时必填（至少选一项），所选桂教通平台/机构在桂教通超管与对应机构的应用上架管理界面新增未上架记录 |

范围选择的具体交互：
- 桂教通超级管理员：是否跨机构=否时全平台（文本提示"全平台所有机构"）/ 按范围（可搜索下拉框多选，数据来超管的上架范围管理）；是否跨机构=是时仅显示"按范围"。
- 机构管理员：全平台/按范围单选、必填（低代码/非低代码均展示）。全平台=上架到当前机构及所有下级机构并自动安装到应用中心；按范围=可搜索下拉框多选（数据来源本机构上架范围管理）。确认上架后按所选范围在应用上架管理新增未上架数据。当前机构为学校时不展示范围选择。
- 分享范围 + 其他机构（仅机构管理员，低代码/非低代码均展示）：分享范围多选（桂教通平台/其他机构，学校机构必填、其余非必填）；勾选"其他机构"时展示机构模糊搜索多选框（输入机构名称模糊筛选、最多 50 条、可多选）。当前机构为学校时：所选"桂教通平台"或"其他机构"在桂教通超管与对应机构的应用上架管理界面各新增一条未上架记录。

（2）确认上架

确认上架后 app.listing_status = listed，toast"上架成功"，返回 P04。

> 上架记录规则：一次上架无论选择多少个范围（含全平台/按范围多选），均只生成一条上架记录；该记录的上架范围列仅聚合展示「范围选择」所选内容（选择全平台则展示"全平台"，选择按范围则展示所选范围名称，英文逗号隔开），不含分享范围；各范围可单独点击进入对应范围的 P04b 应用安装列表页。低代码应用的版本号在 P02b 上线申请模式已配置，本页上架不要求再次选择版本号。机构管理员且当前机构为学校时：无范围选择，上架范围列展示所选分享目标（桂教通平台/其他机构名称），记录在桂教通超管与对应机构的应用上架管理界面生成，状态为未上架。
...
展示当前用户头像、用户名、所属租户名称（由统一门户提供）。提供"我的应用中心""我的订单""教育服务中心"快捷链接卡片。
