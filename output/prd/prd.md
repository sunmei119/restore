...
| 版本 | 日期 | 修订人 | 修订说明 | 状态 |
|------|------|--------|----------|------|
| v2.0 | 2026-07-17 | 孙美 | 重写：适配 design v2.0（范围改名、分类管理重构、审批管理重构、删除共享功能、补充 4 个状态机） | 初稿 |

---

## 一、文档概述

本文档为应用中心重构的产品需求规格说明，面向多机构应用全生命周期管理平台，覆盖机构内应用申请→审批→上架→范围→安装、平台级应用监管与运营统计、教育服务中心集成以及订单管理的完整功能定义。
...
| 模块 | 包含功能 |
|------|---------|
| 应用管理 | 应用申请管理、应用上架管理、全平台应用（超管专属）、上架范围管理、应用分类管理、应用审批管理、标签管理 |
| 平台管理 | 应用统计 |
| 订单管理 | 订单套餐授权管理、订单记录管理（应用购买的订单）、试用套餐记录管理、应用套餐配置 |
| 门户端 | 教育服务中心、付费应用购买页、我的空间、我的应用中心、我的订单 |
...
### 3.1 应用上线申请审批流程
此处管理员指的是桂教通超级管理员，只有桂教通超级管理员具有审批权限。
机构管理员新增应用、上线申请、桂教通超级管理员审批、结果归档三个阶段。

1. 机构管理员在应用申请管理页对已创建但未上线（app.status = offline）的应用提交上线申请。提交前确认应用基本信息完整，提交后系统生成一条审批记录（review.status = pending）。此时应用不可编辑基本信息。
2. 桂教通超级管理员在审批列表的"待审批"Tab 中看到该记录，查看应用详情后选择"通过"或"驳回"。通过后 app.status 变为 online，应用进入机构管理员的应用上架管理列表（P04），状态为未上架（listing_status = unlisted）；驳回时需填写驳回原因，review.status 变为 rejected，app.status 保持 offline。
3. 审批通过后应用可进入上架流程。审批驳回后回到上线申请人，修改后可重新提交上线申请。审批记录永久保留。

```mermaid
flowchart TD
    A["机构管理员：应用申请管理页"] --> B{"应用状态?"}
    B -->|"offline 未上线"| C["提交上线申请<br/>确认基本信息完整"]
    B -->|"online 且 unlisted"| Z["下线独立路径"]
    C --> D["系统生成审批记录<br/>review.status = pending"]
    D --> E["应用基本信息锁定不可编辑"]
    E --> F["桂教通超级管理员在审批列表待审批Tab 查看"]
    F --> G{"审批结论"}
    ...
    D -. "审批期间网络中断" .-> D1["当前结果不落库<br/>已填意见保留可重试"]
```

审批期间网络中断时，当前审批结果不落库，已填写意见保留并允许重试。

下线为独立路径，不经过审批：应用创建人或管理员对 app.status = online 且 app.listing_status = unlisted 的应用点击"下线"，弹窗确认后直接变为 offline；已上架（listing_status = listed）的应用不允许下线，按钮直接隐藏。

### 3.2 应用上架及安装流程

管理员上架应用、配置范围、安装三个环节。

1. 管理员对已上线（online）且未上架（unlisted）的应用发起上架，进入上架配置页，配置上架范围（全平台或按范围选择），P05 展示上架承诺书文案及链接供查阅，须勾选上架承诺书（未勾选时"确认上架"置灰）后方可确认。确认后 app.listing_status 变为 listed。已上架的应用，点击对应的上架范围，打开应用安装列表，点击机构对应的安装按钮为单个安装，点击全部安装则安装到当前安装列表的所有机构下，卸载和全部卸载通用的逻辑；
...
点击免费试用，生成一条试用订单记录；点击立即购买打开订单确认 & 开票信息填写界面，发票信息录入右上角提供"历史发票记录"入口，点击可查看当前人员历史发票记录，选择一条后自动回填至发票信息表单，也可手动填写发票信息；点击提交订单跳转到付费界面，展示跟惠缴费对接的二维码，手机扫码唤起惠缴费半屏支付，支付成功后PC端支付状态同步提示支付成功，支付失败则提示失败，同时出现返回订单的按钮点击返回我的订单界面。PC 端支付状态同步机制：惠缴费完成支付后通过异步回调通知平台后端（回调含签名校验、幂等处理），后端校验通过后更新订单状态为已支付，并触发订单记录、套餐授权、接收记录生成；PC 端支付界面每 3 秒轮询一次订单支付状态接口，检测到已支付即自动提示"支付成功"并进入成功页，检测到失败则提示失败并展示"返回订单"按钮，超过支付有效期（15 分钟，与二维码有效期一致）未支付则提示支付超时、二维码失效可点击"刷新二维码"；不做 WebSocket/SSE 服务端推送
...
本模块负责机构内应用的申请、上架、范围配置、分类管理和审批全流程，包含 12 个页面。

模块级权限规则：默认 tenant_admin 具备本机构应用的管理权限（新增/编辑应用、提交上线申请、上架配置）；platform_admin 和 platform_operator 仅可查看，不可修改机构应用数据；tenant_user 无此模块访问权限。
...
点击"确认上线"前系统校验应用的 Logo、名称、分类等必填项是否完整。校验不通过时顶部红色提示"应用信息不完整，请先完善"，阻止提交。校验通过后系统生成一条审批记录（review.type = 上线，review.status = pending，review.applicant = 当前登录人），toast 提示"上线申请已提交，待桂教通超级管理员审批"，返回列表页。
...
审批中的应用"管理"按钮不置灰，可正常点击进入 P02b 管理模式；编辑应用与更新 SuiteKey 不受审批状态限制，审批中亦可操作。

（5）下线应用

仅 app.status = online 且 app.listing_status = unlisted 时显示"下线"按钮。已上架（listing_status = listed）时该按钮直接隐藏。

点击"下线"弹出确认框"确定要下线该应用吗？"，确认后 app.status = offline，无需审批，不生成审批记录。下线失败时顶部红色提示"操作失败，请检查网络后重试"。
...
| 应用名称 | * | 单行文本框 | 最大 20 字符，右侧实时显示"已输入字数/20"；超限禁止继续输入；名称在同机构内唯一，重复时字段下方红字"该应用名称已存在，请更换" |
...
点击"新建"触发全量校验，未通过时页面滚动定位到首个错误字段。校验通过后写入 app 记录，系统自动生成 app.system_suite_id（全局唯一，不可修改）与 app.system_suite_key，同时自动生成业务应用ID（app.app_id，当前最大应用ID +1、全局唯一）与应用 openId（app.app_open_id：非低代码应用生成长度 15 的英文/数字/下划线串，低代码应用取填写的低代码应用ID），返回 P01 列表。toast 提示"应用创建成功"（新建仅创建应用，上线申请由 P01「上线」按钮触发，不在此处生成审批记录）。
...
提交时全量校验必填项。校验通过后覆盖写入 app 记录（app.system_suite_id 不变）。如属于已上架应用（app.status = online），同步生成一条待审批 review 记录（review.type = 修改，review.status = pending，review.applicant = 当前登录人），该记录进入「应用审批管理（P10）」待审批列表，toast 提示"提交成功，需重新上架审批后生效"，返回 P02b。

> 编辑生效规则（重新上架审核）：审批通过前，对外（门户端、机构管理员应用上架管理等）仍展示修改前的旧值；review.status 变为 approved 后，系统将新值切换为对外展示版本，应用进入机构管理员应用上架管理列表（listing_status = unlisted）。此过程即"重新上架审核"，含义为：已上架应用经编辑后，需经桂教通超级管理员审批方能在线上生效新值。

#### P02b 应用详情页

页面由面包屑、头部凭证区、基本信息区（上线申请模式下基本信息区下方嵌入「上线配置」区，二者同属一个模块）和底部操作区组成。根据进入入口分为四种模式：管理模式（P01"管理"进入，仅 online 应用）、上线申请模式（P01"上线"进入，仅 offline 应用）、只读查看模式（P01 其他用户"查看"进入）、审批详情模式（P10"详情"进入）。
...
| SuiteKey | app.system_suite_key | 标签+值两栏 + 右侧"复制"链接按钮 |

管理模式、上线申请模式均显示 SuiteKey 的"复制"链接（点击复制 app.system_suite_key 值）。上线申请模式展示凭证区（凭证在创建应用时已生成，只读、不可更新）。只读查看模式和审批详情模式显示凭证区但不显示"复制"链接。

（2）复制 SuiteKey

点击 SuiteKey 右侧"复制"链接，系统将 app.system_suite_key 值写入剪贴板，toast 提示"SuiteKey 已复制"。复制失败（如浏览器权限限制）时 toast 红色提示"复制失败，请手动复制"，页面无其它变更。

（3）查看基本信息区

板块标题为"基本信息（修改的信息需要重新上架审批后生效）"，右侧提供"修改"链接按钮（仅管理模式显示）。点击"修改"跳转 P02 编辑模式，字段全部回填。
...
- 数据与授权：是否数据同步（是/否）、用户个人信息授权（是/否）、推送地址、TOKEN、EncodingAesKey（仅数据同步=是时展示推送地址/TOKEN/EncodingAesKey）
...
**上线申请模式**：应用基本信息区下方（同一模块内）新增可编辑「上线配置」区，提交/返回按钮统一置于模块最下方，显示「提交」（主）+「返回」（次）。点击"提交"前系统核对应用必填字段完整性及上线配置必填项，且须勾选上架承诺书（未勾选时"提交"置灰）；缺项时顶部红色提示"应用信息不完整，请先完善"，阻止提交。校验通过后系统生成 review 记录（review.type = 上线，review.status = pending），toast 提示"上线申请已提交，待桂教通超级管理员审批"，返回 P01 列表。

上线配置区（上线申请模式）字段及显示逻辑：
- 是否跨机构（radio 是/否，*）：仅低代码应用展示；桂教通超级管理员选"是"时联动展示跨机构层级与所属机构，机构管理员选"是"时不展示这两项。
- 跨机构层级（radio 厅/市/区县，*）：仅桂教通超级管理员 + 低代码应用 + 是否跨机构=是 时显示。
- 所属机构（可搜索多选输入框，*）：仅桂教通超级管理员 + 低代码应用 + 是否跨机构=是 时显示；根据输入机构名称模糊搜索（最多查询 50 条），支持多选；数据来源按跨机构层级获取——层级为厅→获取所有教育行政部门，为市→获取所有市级教育行政部门，为区县→获取所有区县教育行政部门；跨机构层级切换时清空已选并重新加载候选。
...
该应用已被删除或不属于当前机构时，页面顶部提示"应用不存在或无查看权限"，隐藏所有操作按钮。

（5）查看审批信息（审批详情模式）

审批信息板块展示当前应用最近一条上线审批记录：申请类型（上线/下线/修改）、审批状态徽标（待审批黄色/通过绿色/驳回红色）、申请人、申请时间、审批人、审批时间、审批备注（驳回时高亮显示）。若应用从未提交过审批记录，整块板块隐藏。
...
筛选栏提供应用名称、应用分类、上架范围（下拉，本机构的范围列表）。点击"搜索"按条件刷新列表；点击"重置"清空条件恢复默认视图。

列表按创建时间倒序，每页 20 条，展示：应用 Logo、应用名称、应用分类（一级/二级）、应用类型（平台应用/非平台应用，仅桂教通超级管理员列展示）、开发者名称、应用属性（完整属性文本，如"原生应用""原生应用/低代码应用""生态应用"）、上架范围（仅展示上架时「范围选择」所选内容：选择全平台则展示"全平台"，选择按范围则展示所选范围名称，以英文逗号隔开；不展示分享范围。每条范围均为超链接、可单独点击进入对应范围的 P04b 应用安装列表）、创建时间、上架状态（已上架/已下架/未上架标签）。

> 上架记录规则：一次上架无论选择多少个范围，均只生成一条上架记录（listing_status = unlisted）；上架范围列仅聚合展示「范围选择」所选内容（选择全平台则展示"全平台"，选择按范围则展示所选范围名称），不含分享范围；各范围可点选进入其对应的应用安装列表页（P04b）。

（2）上架应用

listing_status = unlisted 或 dismissed 且 app.status = online 的应用行显示"上架"按钮。点击进入 P05 应用上架配置页。

> 上架补充规则：
> - 当操作人为学校管理员时，P05 不展示"范围选择"，默认上架的范围即本校（学校无下级机构，全平台/按范围不适用）。
> - 其他机构分享过来的应用（来源为其他机构分享），上架时 P05 不展示"分享范围"，被分享过来的应用不可再被分享给其他机构。
> - 上架范围为**动态引用**：应用上架后，其上架范围解析出的机构集合为每次实时解析（非上架时刻快照）。机构、标签、范围任一环节发生增删变化，已上架应用的上架范围自动同步更新，无需重新上架、无需重新审核。

> 已下架（dismissed）应用重新上架时，保留原上架范围：P05 的上架范围默认回填该应用下架前的上架范围配置，管理员可在此基础上调整，确认后直接进入 listed，不丢失原范围设置。
...
停止分享：点击行内"停止分享"弹出"停止分享"弹窗。弹窗内"停止分享范围"为下拉多选，逐一列出该应用上架时分享的每个范围：桂教通平台 + 每个被分享机构名称（如分享时选了 6 个机构，则这 6 个机构逐一列出，必填）。提交后：按所选停止分享范围（机构），将对应机构应用上架管理列表中的该应用下架（若已上架则改为已下架 dismissed，若未上架则保持未上架）；已安装应用不卸载。所选停止分享范围覆盖全部后，该应用从分享列表移除。
...
由 P04"上架"按钮进入，页面分为应用基本信息区（只读）与上架配置区两部分（上架配置区仅保留"范围选择"与"分享范围"；机构管理员且当前机构为学校时，上架配置区不展示"范围选择"，默认上架范围即本校，仅展示"分享范围"；其他机构分享过来的应用上架时不展示"分享范围"，被分享过来的应用不可再被分享给其他机构）；底部按钮居中："取消"和"确认上架"；须勾选上架承诺书（未勾选时"确认上架"置灰）后才可提交。

> - **范围选择**：桂教通超级管理员：全平台/按范围（必填）；机构管理员：全平台/按范围（必填，对低代码/非低代码均展示，全平台=上架到当前机构及所有下级机构并自动安装应用中心）；机构管理员且当前机构为学校时不展示范围选择（学校无下级机构，全平台/按范围不适用），默认上架范围即本校。
> - **分享范围**：仅机构管理员展示（低代码/非低代码均展示）；多选（桂教通平台/其他机构，学校机构必填、其余非必填）；勾选"其他机构"时联动展示机构模糊搜索多选框（输入机构名称模糊筛选、最多 50 条、可多选）。当前机构为学校时：所选"桂教通平台"或"其他机构"会在桂教通超管与对应机构的应用上架管理界面各新增一条未上架记录。其他机构分享过来的应用不展示分享范围（被分享过来的应用不可再被分享给其他机构）。
...
| 字段 | 控件 | 必填 | 联动规则 |
|------|------|:----:|---------|
| 范围选择 | radio 全平台/按范围 | *（机构管理员必填） | 桂教通超级管理员：是否跨机构=否时全平台/按范围（必填），是否跨机构=是时仅按范围；机构管理员：全平台/按范围（必填，对低代码/非低代码均展示）；机构管理员且当前机构为学校时不展示该字段（默认上架范围即本校） |
| 分享范围 | checkbox 桂教通平台/其他机构 | 学校机构必填 | 机构管理员展示（低代码/非低代码均展示）；多选；勾选"其他机构"时联动展示"其他机构"机构多选框（模糊搜索、最多 50 条、可多选）；当前机构为学校时必填（至少选一项），所选桂教通平台/机构在桂教通超管与对应机构的应用上架管理界面新增未上架记录；其他机构分享过来的应用不展示该字段（被分享的应用不可再被分享） |

范围选择的具体交互：
- 桂教通超级管理员：是否跨机构=否时全平台（文本提示"全平台指的是所有机构和学校"）/ 按范围（可搜索下拉框多选，数据来超管的上架范围管理）；是否跨机构=是时仅显示"按范围"。
- 机构管理员：全平台/按范围单选、必填（低代码/非低代码均展示）。全平台=上架到当前机构及所有下级机构并自动安装到应用中心；按范围=可搜索下拉框多选（数据来源本机构上架范围管理）。确认上架后按所选范围在应用上架管理新增未上架数据。当前机构为学校时不展示范围选择，默认上架范围即本校。
- 分享范围 + 其他机构（仅机构管理员，低代码/非低代码均展示）：分享范围多选（桂教通平台/其他机构，学校机构必填、其余非必填）；勾选"其他机构"时展示机构模糊搜索多选框（输入机构名称模糊筛选、最多 50 条、可多选）。当前机构为学校时：所选"桂教通平台"或"其他机构"在桂教通超管与对应机构的应用上架管理界面各新增一条未上架记录。其他机构分享过来的应用不展示分享范围（被分享过来的应用不可再被分享给其他机构）。

（2）确认上架

须勾选上架承诺书（checkbox，文案"我已仔细阅读并完全同意应用免责声明及上架承诺书，自愿承担一切相关法律责任。"，点击链接弹出免责声明弹窗，未勾选时"确认上架"按钮置灰）后方可提交。

确认上架后 app.listing_status = listed，toast"上架成功"，返回 P04。

> 范围动态引用规则：上架范围为动态引用，不是上架时刻快照。机构、标签、范围增删后，已上架应用的上架范围自动同步更新；仅从范围中移除机构时，应用上架状态不变，只是该机构不再可见该应用（已安装应用不卸载，无需重新审核）。

> 上架记录规则：一次上架无论选择多少个范围（含全平台/按范围多选），均只生成一条上架记录；该记录的上架范围列仅聚合展示「范围选择」所选内容（选择全平台则展示"全平台"，选择按范围则展示所选范围名称，英文逗号隔开），不含分享范围；各范围可单独点击进入对应范围的 P04b 应用安装列表页。低代码应用的版本号在 P02b 上线申请模式已配置，本页上架不要求再次选择版本号。机构管理员且当前机构为学校时：无范围选择，默认上架范围即本校，上架范围列展示本校及所选分享目标（桂教通平台/其他机构名称），记录在桂教通超管与对应机构的应用上架管理界面生成，状态为未上架。其他机构分享过来的应用上架后不生成分享记录（被分享过来的应用不可再被分享给其他机构）。

#### P06 全平台应用页

桂教通超级管理员专属页面，跨机构查看**全平台已上架应用**并支持强制下架治理。仅超管可见（tenant_admin 不可访问）。菜单入口：应用管理 → 全平台应用。
...
数据范围：仅 `app.listing_status = listed` 的应用（跨全部机构）。列如下：
...
- 点击弹窗二次确认："确定要强制下架【{app.name}】吗？下架后该应用在【{app.tenant_name}】的上架列表将变为已下架状态，已安装的应用不卸载。"
- 确认后：
  - `app.listing_status = dismissed`（回到已下架）
  - 已安装应用不卸载（沿用机构下架规则）
  - 写入 OperationLog：`log.action = force_delist`、`log.operator_role = platform_admin`，detail 记录被下架应用、原上架机构、执行人
  - toast「强制下架成功」，列表刷新（该条因不满足 listed 条件被移出）
- 对应机构管理员在 P04 应用上架管理页看到该应用状态由"已上架"回落为"已下架"，分享到的其他机构对应的应用状态也改为已下架，被分享的机构在分享机构未上架前，不可上架，若分享的机构重新上架后分享范围有变更，以最新的分享范围为准，历史的范围如果不在新范围内的，将对应机构的应用删除，如如强制下架前分享给了A机构，重新上架后不分享A机构，则AI机构在对应的上架管理列表将删除该应用
- 取消：不变更。

（4）说明

- 本页不提供其他跨机构操作（无编辑、无审批、无上架），仅治理性下架
...
- 删除保护：被应用上架范围引用的范围不可删除。删除前校验该范围是否被已上架（listed）应用使用，被引用时提示"该范围已被 N 个应用的上架范围引用，不可删除"。
...
- 动态引用说明：范围机构列表为**动态解析**结果（每次实时计算），机构新增/删除后由「同步范围机构」按钮（或标签联动）自动补齐/移除，使用该范围的应用上架范围同步更新。
...
> 保存后自动同步：范围保存后，使用该范围的应用，其上架范围自动同步更新（动态引用，无需重新上架）。范围为"按标签"类型时，后续标签中机构的增删自动反映到该范围，并继续同步到使用该范围的应用上架范围。
...
停用规则：已停用分类在已使用场景中展示正常——应用列表的分类展示、分类筛选（筛选条件中仍可选该分类）、应用详情中的分类展示均不受影响；仅在应用新增（选择所属分类）时不展示已停用分类。

（6）删除分类

仅已停用（status = disabled）的分类行显示"删除"按钮。已启用分类不显示删除按钮。

删除限制：已被应用使用的分类不可删除；存在下级分类的分类不可删除（需先删除其下级分类，且下级分类均无应用使用）。

满足删除条件时，点击"删除"弹窗二次确认"数据删除后，无法找回，请确认是否继续删除？"，确认后删除该分类。

#### P10 应用审批列表页

页面顶部为两个 Tab 切换："待审批"和"已审批"。筛选栏提供应用名称、应用分类、应用 ID 三类查询条件，支持搜索和重置按钮。

查询条件说明：
- 应用名称：模糊查询；
- 应用分类：级联下拉（一级/二级），按分类路径过滤；
- 应用 ID：文本框，精确匹配业务应用 ID。

（1）查看待审批列表

筛选条件：应用名称（模糊查询）、应用分类（级联下拉）、应用 ID（精确匹配），支持搜索和重置。列表展示字段复用「应用申请管理（P01）」列表（见 P01（1）），即：应用 Logo、应用名称、应用 ID、应用分类、应用属性、开发者名称、申请时间、类型标签（标识待办类型：上线/修改），**不含操作列**。按申请时间倒序排列，每页 20 条。底部显示分页信息。

操作列：显示"审批"按钮（仅 review.status = pending 时显示）和"查看"按钮（任意状态均可点击）。

（2）查看已审批列表

筛选条件同待审批。列表展示字段复用「应用申请管理（P01）」列表，即：应用 Logo、应用名称、应用 ID、应用分类、应用属性、开发者名称、申请时间、类型标签（上线/修改），**不含操作列**，并附加：审批时间、审批结果标签（通过绿色/驳回红色）、审批人。

操作列仅显示"查看"按钮。

#### P11 应用审批操作页（审批模式）

由 P10 待审批 Tab 行尾"审批"按钮进入。页面分两张卡片：上方卡片展示应用完整信息（全部只读），下方卡片展示审批操作区。底部按钮居中。
...
- 点击"提交"：审批结果=通过时 review.status = approved，app.status = online，应用进入机构管理员的应用上架管理列表（listing_status = unlisted），写入 reviewer 和 reviewed_at，toast「审批通过，应用已进入机构管理员上架列表（未上架）」；审批结果=驳回时 review.status = rejected，app.status 保持 offline，review_comment = 审批意见，toast「已驳回」。
...
| 审批人 | review.reviewer（未审批时显示"---"） |
| 审批时间 | review.reviewed_at（未审批时显示"---"） |
| 审批结果 | review.status el-tag 标签（通过绿色/驳回红色），未审批时显示"---" |
...
- 列表列：机构名称、机构 ID、机构类型。
- 分页：底部分页器（每页 10 条，支持翻页与总条数展示），查询条件变化后回到第 1 页重新分页。
- 操作按钮：查询区右侧提供「同步标签机构」按钮。点击后调用后端接口校验当前标签是否已包含全部机构：
  - 若未全部包含，后端自动将遗漏的机构补足到当前标签，前端刷新列表并提示「已同步标签机构，新增 N 个机构」；
  - 若已全部包含，提示「当前标签机构已完整，无需同步」。
- 动态引用说明：标签机构列表为**动态解析**结果（每次实时计算），机构新增/删除后由「同步标签机构」自动补齐/移除，自动反映到引用该标签的范围及应用上架范围。
...
> 修改后自动同步：标签可见范围（机构）增删后，引用该标签的所有范围自动同步添加/移除对应机构，使用这些范围的应用上架范围同步更新（动态引用，无需重新上架）。

（4）删除标签

被范围引用的标签（关联范围数量 > 0）隐藏删除按钮，不可删除（删除前需确认是否被使用）。未被引用时可删除，弹窗二次确认后删除。删除后标签从关联范围可见范围自动移除（关联范围及应用上架范围同步变化）。

### （二）平台管理模块

本模块负责运营统计，包含 1 个页面。platform_admin、platform_operator 可访问并查看全平台统计数据；机构管理员（tenant_admin）可访问并查看本机构统计数据。
...
> 统计口径按角色区分：
> - 桂教通超级管理员（platform_admin）/ 平台运营员（platform_operator）：统计**全平台**数据，含平台应用和平台管理的机构应用（独立机构应用不计入）。
> - 机构管理员（tenant_admin）：统计**本机构**数据，即本机构创建/管理的应用及本机构范围内的使用数据。
...
4 张指标卡片 4 等分铺满内容容器，按角色区分口径：
- 超管（platform_admin / platform_operator）：应用安装数（平台应用安装成功数量，累计至当前）、平台应用数（超管上架应用总数，累计至当前）、活跃应用数（时间范围内至少打开一次的应用数）、应用使用人数（时间范围内至少使用一次平台任意应用的用户总数）。除活跃应用数、应用使用人数随时间切换变化外，其余卡片不随时间切换变化。
- 机构管理员（tenant_admin）：应用安装数（本机构范围内应用安装成功数量，累计至当前）、机构应用数（本机构上架应用数，累计至当前）、活跃应用数（本机构范围内时间范围内至少打开一次的应用数）、应用使用人数（本机构范围内时间范围内至少使用一次应用的用户总数）。

（3）图表区（三图一行等分）

三个图表卡片在同一行等宽展示，数据范围随角色切换（超管为全平台、机构管理员为本机构范围）：

- 应用点击量趋势：折线图，展示时段点击量累计与趋势（超管为全平台点击量，机构管理员为本机构范围内点击量）
- 分类统计：环形图，统计各分类的应用数（超管为平台分类统计，机构管理员为本机构应用分类统计）
- 活跃/非活跃应用占比：环形图，展示活跃应用与非活跃应用的占比（超管为全平台应用，机构管理员为本机构应用）

（4）排行榜区（两榜一行）

两个表格榜单在同一行展示，数据范围随角色切换（超管为全平台、机构管理员为本机构范围）：
...
点击右上角"导出"按钮，生成 1 个 Excel 文件（.xlsx），内含 6 张工作表，与页面各统计区块一一对应：

| 工作表 | 对应区块 | 字段 |
|--------|----------|------|
| 统计概览 | 指标卡片区 | 统计角色、统计范围、时间范围、应用安装数、平台应用数/机构应用数、活跃应用数、应用使用人数、导出时间 |
| 应用安装统计 | 应用安装数指标 | 应用名称、应用ID、应用类型（平台应用/非平台应用）、所属机构、应用分类（一级/二级）、安装机构数、安装数量（累计至当前）、最近安装时间 |
| 应用使用统计 | 活跃应用数/应用使用人数/点击量趋势 | 应用名称、应用ID、所属机构、活跃状态（活跃/非活跃）、使用人数（周期内）、使用次数（周期内）、点击量（周期内） |
| 应用分类统计 | 分类统计环形图 | 一级分类、二级分类、应用数、占比 |
| 用户数量榜 Top10 | 用户数量榜 | 排名、应用名称、用户数（周期内打开或点击过应用的用户数） |
| 应用安装排行榜 Top10 | 应用安装排行榜 | 排名、应用名称、安装数 |

导出规则：数据范围按角色——超管统计全平台（仅平台应用和平台管理的机构应用，独立机构应用不计入），机构管理员仅本机构范围；时间范围与页面当前选择一致（今日/本周/本月/本年）；文件名为「应用统计_YYYY-MM-DD.xlsx」；导出需具备查看权限。

统计范围说明：超管统计范围仅包含平台应用和平台管理的机构应用（独立机构应用不计入）；机构管理员统计本机构创建/管理的应用及本机构范围内的使用数据。

（6）数据实时性 / 刷新频率

统计采用"实时 + 准实时（分钟级）"混合模式，不做 T+1，所有指标在"今日"周期下即可查看当日数据：

- **刷新时机**：进入页面、切换时间范围（今日/本周/本月/本年）、切换角色时实时向后端拉取最新统计数据；页面提供「刷新」按钮，点击后重新拉取全部区块数据；不做自动轮询。
- **实时指标（秒级）**：应用安装数、平台应用数/机构应用数、应用分类统计、活跃/非活跃应用占比——基于应用/机构/上架等基础业务表实时聚合。
- **准实时指标（最长延迟 5 分钟）**：应用点击量趋势、应用使用人数、活跃应用数、用户数量榜 Top10、应用安装排行榜 Top10——基于点击/使用/安装行为流水，按 5 分钟粒度窗口汇总落库，页面展示最近一次聚合结果。
- **历史周期数据**：本周/本月/本年由日汇总表累加，已结算日期数据固定，当日部分按上述实时/准实时口径展示。
- **数据一致性**：同一时刻结构类与行为类指标可能存在最长 5 分钟时间差，属正常现象；导出数据与页面展示同源、口径一致。
- **结构类与行为类指标划分**：结构类指标（应用安装数、平台应用数/机构应用数、应用分类统计、活跃/非活跃应用占比）描述存量/状态，数据来源于应用、机构、上架、分类等基础业务表，仅在上架/下架、机构入驻/退出、分类调整等低频业务操作时变化，每次请求实时聚合即可秒级返回；行为类指标（应用点击量趋势、应用使用人数、活跃应用数、用户数量榜 Top10、应用安装排行榜 Top10）描述流量/活动，数据来源于点击、打开、安装等行为流水表（每次用户操作产生一条），量级大，需按 5 分钟窗口预聚合落库，页面读取最近一次聚合结果。

### （三）订单管理模块

本模块负责订单套餐授权、订单记录（应用购买的订单）、试用订单和应用套餐配置管理，包含 5 个页面。

模块级权限规则：platform_admin 全部管理（授权/取消授权/手动授权/新增授权、停用/延期等操作）；platform_operator 全部只读；tenant_admin 本机构内查看；套餐配置页 tenant_admin 可管理本机构套餐。
...
筛选栏：订单ID（模糊查询）、套餐名称（模糊查询）、业务套餐（下拉：全部/短信充值/存储容量充值/TOKEN充值/基座套餐/应用使用授权/应用试用授权）、所属机构（模糊查询）。列表展示：订单 ID、授权方式（自动/手动标签）、套餐名称、使用范围、业务套餐、有效期/天、分配大小、所属机构、购买用户 ID、到期时间、订单接收时间、授权状态（授权成功/未授权/授权失败标签）、授权状态描述。
...
- **手动授权**（授权方式=自动且 authStatus=failed 时显示）：自动授权失败的订单，操作列显示「手动授权」按钮；点击弹窗二次确认「自动授权失败，确认手动授权【套餐名称】吗？」，确认后授权成功，授权状态变为授权成功、授权状态描述变为已手动授权，toast「手动授权成功」；点击取消不变更。

（3）新增授权

列表页右上角提供主题蓝色「新增授权」按钮，点击弹窗，表单字段及顺序为：订单 ID、授权方式、业务套餐（下拉，必填）、套餐名称（下拉，根据所选业务套餐加载对应类型套餐，必填）、使用范围、有效期/天、分配大小、所属机构、购买用户 ID、到期时间、订单接收时间、授权状态、授权状态描述。除使用范围、分配大小、授权状态描述为选填外，其余字段均为必填。确认后新授权记录插入列表顶部。

#### P19 订单记录管理列表页

订单记录管理记录两类订单：①在桂教通平台购买的付费应用订单（教育服务中心/我的应用中心内嵌商城商品页下单）；②通过商城下单的订单（商城嵌入式页面/无痕下单）。列表列较多，支持横向滚动。

（1）查询和列表

筛选栏：订单 ID（模糊查询）、套餐名称（模糊查询）、购买用户 ID（模糊查询）、支付状态（下拉：全部/待支付/已支付/已过期/已取消）。
...
筛选栏：套餐名称、套餐编码、业务套餐（下拉）。列表展示：序号、业务套餐、套餐编码、套餐名称、价格、分配大小、计量单位、有效期(天)、基座套餐关联、是否手动开通业务。列表内容较宽，支持横向滚动。
...
| 字段 | 必填 | 控件 | 联动规则 |
|------|:----:|------|---------|
| 业务套餐 | * | 下拉 | 数据来源数据字典：短信充值/存储容量充值/TOKEN充值/基座套餐/应用使用授权/应用试用授权 |
| 套餐编码 | * | 文本框 | 常显；全局唯一校验 |
| 套餐名称 | * | 文本框 | 常显 |
| 价格(元) | * | 数字输入框 | 常显；必填 |
...
| 适用范围 |   | 文本框 | 常显；非必填。业务套餐为应用使用授权、应用试用授权时填写应用 ID |
| 基座套餐关联 | 条件必填 | 下拉多选 | 业务套餐=基座套餐时必填；数据源为已存在的套餐配置，可多选 |
| 套餐适用对象 |   | 下拉选择 | 字典：机构/个人 |
| 是否手动开通业务 | * | radio | 自动/手动 |

编辑时回填已有数据。底部按钮居中："取消"和"确定保存"。保存时校验必填项（含价格）、套餐编码全局唯一，并按业务套餐校验分配大小、有效期、基座套餐关联。

**编辑约束（被引用套餐）**：套餐被引用（订单套餐授权记录、订单记录、试用套餐记录、其他套餐的基座套餐关联、应用上架配置引用）时仍允许编辑，但**编辑仅影响新购**——保存后仅对之后新产生的订单/授权/试用按新配置生效；**历史订单沿用下单时的套餐快照**（下单时套餐名称、价格、分配大小、计量单位、有效期等字段已快照至订单），不受后续编辑影响；应用上架配置中引用的授权/试用套餐同样仅对新购生效。
...
本模块覆盖最终用户可见的教育服务中心、应用详情、应用购买。根据桂教通超管应用上架范围、试用学段、可见性配置来展示教育服务中心的应用

#### P21 教育服务中心页

> 教育服务中心（P21 我的空间 / P22 桂教通门户）由「轮播 Banner（桂教通服务中心）」与「成长服务范围」两个独立模块组成，模块下方为服务类型 Tab 与应用列表。轮播 Banner 两端均展示；**成长服务范围仅桂教通门户（P22）展示，我的空间（P21）不展示**。P21 我的空间-教育服务中心内容铺满容器宽度；P22 桂教通门户-教育服务中心保持 1200px 居中。
...
- 免费应用：若没有安装，展示免费安装，点击后系统自动安装，免费安装按钮loading中，安装成功后，按钮显示立即打开；若已经安装，按钮显示立即打开。
- 付费应用：若没有购买，详情页展示该应用的授权套餐列表（多个套餐卡片，含套餐名称与价格），默认选中第一个套餐，点击不同套餐切换选中并联动展示对应价格；"立即购买"按钮展示所选套餐价格（如"立即购买 ¥199"），若应用为可试用，同时显示免费试用按钮。点击立即购买进入订单确认 & 开票信息填写页面（展示所选套餐及对应单价），发票信息录入右上角提供"历史发票记录"入口，点击弹窗展示当前人员历史发票记录列表（弹窗支持分页，每页 5 条），选择一条后自动回填至发票信息表单；完成购买信息填写后，到支付界面，支付的微信二维码扫码唤起惠缴费的半屏支付界面，支付成功后，生成一条订单记录到订单管理模块的订单记录管理列表页，同时用户的我的空间-我的订单也生成对应的订单记录。支付结果同步：手机支付完成后惠缴费异步回调平台后端确认（签名校验、幂等），支付界面每 3 秒自动轮询订单支付状态，支付成功即自动提示"支付成功"并展示成功页；支付失败提示失败并展示"返回订单"按钮；超过支付有效期（15 分钟，与二维码有效期一致）未支付则提示支付超时、二维码失效可点击"刷新二维码"；不做 WebSocket/SSE 服务端推送。
点击免费试用，后端将当前用户新增到使用机构，同时以试用机构的打开使用应用。
...
3. **我的应用 & 低代码应用（第一行合并模块，Tab 切换）**：
   ...
   - 低代码应用 Tab 展示当前用户所有已安装的低代码应用及自建应用，提供分类切换（全部、教学类、行政类、科研类），切换不同分类展示对应应用（两行图标卡片），默认"全部"；低代码应用若为跨机构应用，图标卡片下方显示来源机构名称标签。
...
顶部提供订单号搜索框。列表以卡片形式展示当前用户/机构的订单：状态标签、订单号、下单时间、实付金额（红色）、商品图标、商品名称、单价×数量，"支付"按钮（状态为待支付且未超时的订单显示，点击跳转 P22 付款界面微信扫码支付）以及"订单详情"按钮。卡片按下单时间倒序。

（2）订单详情

点击"订单详情"进入详情视图（"<< 返回订单列表"返回）。状态为待支付且未超时的订单，详情页顶部展示"支付"按钮，点击跳转 P22 付款界面（微信扫码支付）。详情页按下图结构展示：
...
flowchart TD
    ...
    B --> B1["订单 ID"]
    B --> B2["付款状态：el-tag 标签"]
    C --> C1["商品信息区（展示风格同发票信息）：应用名称 / 套餐名称 / 价格 / 购买类型 / 数量 / 实付金额"]
    D --> D1["发票类型 / 纳税人识别号 / 发票抬头 / 发票内容 / 单位名称 / 邮箱"]
```

- **订单信息**：订单 ID、付款状态（el-tag 标签，如已支付/待支付/已取消）。
- **商品信息**：应用名称、套餐名称、价格、购买类型（个人/机构）、数量、实付金额（展示风格同发票信息，标签+值网格）。
- **发票信息**：发票类型、纳税人识别号、发票抬头、发票内容、单位名称、邮箱（无数据时显示"—"）。
...
|------|------|---------------|-------------------|-------------|-------------|
| 应用管理 | 应用申请管理页 | 可操作（全平台） | 全部只读 | 本机构管理 | — |
| | 应用申请编辑页 | 可操作（全平台） | — | 本机构新增/编辑 | — |
| | 应用上架管理页 | 可操作（全平台） | 全部只读 | 本机构管理 | — |
| | 应用安装列表页 | 可操作（全平台） | 全部只读 | 本机构管理 | — |
| | 上架范围管理页 | 全部只读 | 全部只读 | 本机构管理（自定义范围管理，默认范围只读） | — |
| | 上架范围编辑页 | — | — | 本机构编辑（自定义范围） | — |
...
| | 应用审批操作页 | 全部审批 | — | — | — |
| | 标签管理页 | — | — | 本机构管理 | — |
| 平台管理 | 应用统计看板页 | 全部查看/导出 | 全部查看/导出 | 本机构查看/导出 | — |
| 订单管理 | 订单套餐授权管理 | 全部管理 | 全部只读 | 本机构查看 | — |
| | 订单记录管理 | 全部只读 | 全部只读 | 本机构查看 | — |
| | 试用套餐记录管理 | 全部管理 | 全部只读 | 本机构查看 | — |
| | 应用套餐配置列表页 | 全部管理 | 全部管理 | 本机构管理 | — |
| 门户端 | 教育服务中心页 | 可见所有 | 可见所有 | 可见机构范围内 | 可见本机构范围 |
| | 应用购买详情页 | 可购买 | 可购买 | 可购买 | 可购买 |
| | 我的应用中心页 | 可用 | 可用 | 可用 | 可用 |
| | 我的订单页 | 可见全部 | 可见全部 | 本机构可见 | 本人可见 |
...
| 桂教通超级管理员 | 应用管理、应用申请管理、应用上架管理、全平台应用、上架范围管理、应用分类管理、应用审批管理、标签管理、应用统计看板、订单套餐授权、订单记录、试用订单记录、应用套餐配置 |

> 说明：订单管理（订单套餐授权/订单记录/试用订单记录/应用套餐配置）仅桂教通超级管理员可见；应用审批管理仅桂教通超级管理员可见；桂教通门户（教育服务中心）、我的空间（教育服务中心/我的应用中心/我的订单）仅机构管理员可见。

### 5.2 按钮级权限

| 页面 | 按钮/操作 | platform_admin | platform_operator | tenant_admin | tenant_user |
|------|-----------|---------------|-------------------|-------------|-------------|
| 应用申请管理页 | 新增/管理/上线/下线 | 可操作（全平台） | — | 本机构可操作 | — |
| 应用申请编辑页 | 保存/提交 | 可操作（全平台） | — | 本机构可操作 | — |
| 应用上架管理页 | 上架/下架/安装 | 可操作（全平台） | — | 本机构可操作 | — |
| 应用审批操作页 | 通过/驳回 | 可审批 | — | — | — |
| 上架范围管理页 | 创建范围/管理 | — | — | 本机构可操作（自定义范围） | — |
| 应用统计看板页 | 导出 | 可导出 | 可导出 | 可导出（本机构范围） | — |
| 教育服务中心 | 安装/购买 | 可操作 | 可操作 | 可操作 | 可操作 |

### 5.3 字段级权限例外

**应用审批操作页（P11）**：
- platform_admin：可审批所有机构提交的上线申请
- 机构管理员（tenant_admin）无审批权限，应用审批管理模块对其不可见

**应用统计看板页（P17）**：
- platform_admin / platform_operator：统计全平台数据，统计范围仅包含平台应用和平台管理的机构应用，独立机构应用不计入
- tenant_admin（机构管理员）：统计本机构数据（本机构创建/管理的应用及本机构范围内的使用数据），导出仅限本机构范围
- 导出功能需具备查看权限方可操作

**上架范围编辑页（P08）**：
- tenant_admin：仅可编辑本机构的自定义范围（新增/修改/删除）
- 默认范围对所有角色只读，无编辑操作

**应用申请编辑页（P02）/ 应用详情页-上线配置（P02b）**：
- 应用类型（平台应用/非平台应用）：仅桂教通超级管理员展示且必填；机构管理员不展示此字段，新增应用默认非平台应用
- 上线配置-跨机构层级、所属机构：仅桂教通超级管理员且低代码应用时展示，且仅"是否跨机构 = 是"时显示；机构管理员展示"是否跨机构"（仅低代码应用）但不展示跨机构层级与所属机构
- 说明：推送地址/TOKEN/EncodingAesKey（是否数据同步=是）、suiteID（应用属性=生态应用）、应用ID（应用属性=低代码应用）、可见性地址（打开方式含对应方式）等为字段间业务联动，非角色权限差异，不在此列

**应用上架配置页（P05）**：
- 范围选择：桂教通超级管理员展示（是否跨机构=否时全平台/按范围，=是时仅按范围）；机构管理员展示（全平台/按范围，必填）；机构管理员且当前机构为学校时不展示该字段，默认上架范围即本校
- 分享范围：仅机构管理员展示（桂教通平台/其他机构多选，学校机构必填、其余非必填）；桂教通超级管理员不展示；其他机构分享过来的应用不展示该字段（被分享的应用不可再被分享）
...
| app.school_levels | string | 否 | school_levels_mode=partial 时必填；多值枚举：kindergarten / primary / junior / senior / vocational_secondary / vocational_higher / undergraduate / graduate |
...
| app.listing_status | enum | 是 | listed / unlisted / dismissed |
| app.tenant_id | string | 是 | 归属机构 |
| app.tenant_name | string | 是 | 归属机构名称；由 app.tenant_id 反查组织中心带出，冗余展示（P06 全平台应用页"上架机构"列） |
| app.suite_id | string | 否 | 生态应用 suiteID，最大 64 字符 |
| app.system_suite_id | string | 是 | 系统级 SuiteId，全局唯一，创建后不可修改 |
| app.system_suite_key | string | 是 | 系统级 SuiteKey，创建应用时自动生成，可在 P02b 凭证区通过"复制"复制其值，不可手动更新 |
...
| category.tenant_id | string | 是 | 归属机构 |
...
| tag.tenant_id | string | 是 | 归属机构 |

### 6.5 审批记录（ReviewRecord）

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| review.id | string | 是 | 审批记录唯一标识 |
| review.app_id | string | 是 | 被审批应用 ID |
| review.type | enum | 是 | online / offline / modify（应用信息编辑后重新上架审批） |
| review.status | enum | 是 | pending / approved / rejected |
| review.applicant | string | 是 | 申请人 |
| review.reviewer | string | 否 | 审批人，审批通过时写入 |
| review.review_comment | string | 否 | 审批意见/驳回原因，最大 200 字符；驳回时必填 |
| review.reviewed_at | datetime | 否 | 审批时间 |
...
| package.base_pkg_id | array<string> | 否 | 基座套餐关联（可多选）；业务套餐=基座套餐时必填 |
...
|------|------|--------|----------|----------|----------|
| offline | 未上线，不可上架 | tenant_admin | 提交上线审批 | online | 审批通过后自动变更 |
...
| 状态 | 含义 | 操作人 | 触发动作 | 下一状态 | 限制条件 |
|------|------|--------|----------|----------|----------|
| unlisted | 未上架，范围中不可见 | tenant_admin | 上架配置 | listed | app.status 必须为 online；需配置上架范围；须勾选上架承诺书（未勾选时确认上架置灰） |
| listed | 已上架，范围中可见 | tenant_admin | 下架 | dismissed | 二次确认："下架后全区用户将无法使用此应用，已安装的应用不会被卸载" |
| listed | 已上架，范围中可见 | platform_admin | 强制下架（P06） | dismissed | 二次确认："确定要强制下架【{app.name}】吗？下架后该应用在【{app.tenant_name}】的上架列表将变为已下架状态，已安装的应用不卸载。"；仅超管可操作；写 OperationLog（force_delist） |
| dismissed | 已下架，范围中不可见，已安装应用不受影响 | tenant_admin | 上架配置 | listed | 可重新上架，且重新上架时保留原上架范围（P05 默认回填下架前的范围配置） |

应用必须先上线（online）后才可上架（listed）。已上架不允许直接下线，必须先下架方可下线。

> 范围动态引用不影响状态：标签或范围内机构的增删仅改变应用的上架范围（可见机构集合），应用上架状态（listing_status）不变，仍在范围内的机构继续可见，不在范围内的机构不可见该应用；已安装应用不卸载、无需重新审核。

### 7.3 审批记录状态机

| 状态 | 含义 | 操作人 | 触发动作 | 下一状态 | 限制条件 |
|------|------|--------|----------|----------|----------|
| pending | 待审批（上线） | tenant_admin | 提交上线申请 | — | app 信息完整方可提交 |
| pending | 待审批（修改） | tenant_admin | 编辑已上架应用并提交 | — | app.status = online；审批通过前对外仍展示旧值 |
| pending | | platform_admin（桂教通超级管理员） | 审批通过 | approved | — |
| pending | | platform_admin（桂教通超级管理员） | 驳回 | rejected | 驳回原因必填 |
| approved | 审批通过，应用进入机构管理员上架列表（未上架）；修改类型审批通过后新值对外生效 | — | — | — | 终态；app.status 自动变为 online，应用进入机构管理员应用上架管理列表（listing_status = unlisted） |
| rejected | 审批驳回 | tenant_admin | 重新提交 | pending | 修改后可重新提交上线/修改申请 |
### 7.4 订单状态机
