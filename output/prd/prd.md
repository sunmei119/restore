...
> - **低代码应用 + 桂教通超级管理员**：展示 局校共用（选"是"展示跨租户层级、所属租户，且范围选择仅显示"按范围"；选"否"范围选择显示"全平台 / 按范围"）/ 范围选择（全平台 / 按范围）。局校共用指将应用上架范围与跨租户范围保持一致，并自动安装到范围内租户（仅安装到应用中心）。
> - **低代码应用 + 租户管理员**：展示 局校共用（选"是"时不展示范围选择；选"否"展示范围选择：全平台/按范围，必填）/ 范围选择（全平台 / 按范围，数据来源本租户上架范围管理）/ 分享范围（多选：桂教通平台/其他机构，非必填；勾选后展示版本号）。不展示数据同步、应用所属厂商、是否自动安装、是否可试用、收费方式及其子项。
...
| 字段 | 控件 | 必填 | 联动规则 |
|------|------|:----:|---------|
| 局校共用 | radio 是/否 | * | 仅低代码应用展示；桂教通超级管理员选"是"时展示跨租户层级和所属租户，租户管理员选"是"时不展示这两项 |
